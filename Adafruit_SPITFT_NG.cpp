...
// Possible values for Adafruit_SPITFT_NG.connection:
...
/*!
    @brief   Adafruit_SPITFT_NG constructor for software (bitbang) SPI.
...
*/
Adafruit_SPITFT_NG::Adafruit_SPITFT_NG(uint16_t w, uint16_t h, int8_t cs, int8_t dc,
                                 int8_t mosi, int8_t sck, int8_t rst,
                                 int8_t miso)
    : Adafruit_GFX_NG(w, h), connection(TFT_SOFT_SPI), _rst(rst), _cs(cs),
      _dc(dc) {
  swspi._sck = sck;
  swspi._mosi = mosi;
  swspi._miso = miso;
...
/*!
    @brief   Adafruit_SPITFT_NG constructor for hardware SPI using the board's
...
Adafruit_SPITFT_NG::Adafruit_SPITFT_NG(uint16_t w, uint16_t h, int8_t cs, int8_t dc,
                                 int8_t rst)
    : Adafruit_SPITFT_NG(w, h, &SPI, cs, dc, rst) {
...
/*!
    @brief   Adafruit_SPITFT_NG constructor for hardware SPI using a specific
             SPI peripheral.
...
*/
Adafruit_SPITFT_NG::Adafruit_SPITFT_NG(uint16_t w, uint16_t h, SPIClass *spiClass,
                                 int8_t cs, int8_t dc, int8_t rst)
    : Adafruit_GFX_NG(w, h), connection(TFT_HARD_SPI), _rst(rst), _cs(cs),
      _dc(dc) {
  hwspi._spi = spiClass;
#if defined(USE_FAST_PINIO)
#if defined(HAS_PORT_SET_CLR)
...
/*!
    @brief   Adafruit_SPITFT_NG constructor for parallel display connection.
    @param   w         Display width in pixels at default rotation (0).
    @param   h         Display height in pixels at default rotation (0).
...
*/
Adafruit_SPITFT_NG::Adafruit_SPITFT_NG(uint16_t w, uint16_t h, tftBusWidth busWidth,
                                 int8_t d0, int8_t wr, int8_t dc, int8_t cs,
                                 int8_t rst, int8_t rd)
    : Adafruit_GFX_NG(w, h), connection(TFT_PARALLEL), _rst(rst), _cs(cs),
      _dc(dc) {
  tft8._d0 = d0;
  tft8._wr = wr;
  tft8._rd = rd;
...
*/
void Adafruit_SPITFT_NG::initSPI(uint32_t freq, uint8_t spiMode) {
...
*/
void Adafruit_SPITFT_NG::setSPISpeed(uint32_t freq) {
#if defined(SPI_HAS_TRANSACTION)
...
*/
void Adafruit_SPITFT_NG::startWrite(void) {
...
/*!
    @brief  Call after issuing command(s) or data to display. Performs
            ...
            for all display types; not an SPI-specific function.
*/
void Adafruit_SPITFT_NG::endWrite(void) {
  if (_cs >= 0)
    SPI_CS_HIGH();
  SPI_END_TRANSACTION();
}
...
*/
void Adafruit_SPITFT_NG::writePixel(int16_t x, int16_t y, uint16_t color) {
...
*/
void Adafruit_SPITFT_NG::swapBytes(uint16_t *src, uint32_t len, uint16_t *dest) {
  if (!dest)
...
*/
void Adafruit_SPITFT_NG::writePixels(uint16_t *colors, uint32_t len, bool block,
                                  bool bigEndian) {
...
*/
void Adafruit_SPITFT_NG::dmaWait(void) {
#if defined(USE_SPI_DMA) && (defined(__SAMD51__) || defined(ARDUINO_SAMD_ZERO))
  while (dma_busy)
    ;
#if defined(__SAMD51__) || defined(ARDUINO_SAMD_ZERO)
...
*/
void Adafruit_SPITFT_NG::writeColor(uint16_t color, uint32_t len) {
...
*/
void Adafruit_SPITFT_NG::writeFillRect(int16_t x, int16_t y, int16_t w, int16_t h,
                                    uint16_t color) {
...
*/
void inline Adafruit_SPITFT_NG::writeFastHLine(int16_t x, int16_t y, int16_t w,
                                            uint16_t color) {
...
/*!
    ...
    @param  color  16-bit line color in '565' RGB format.
*/
void inline Adafruit_SPITFT_NG::writeFastVLine(int16_t x, int16_t y, int16_t h,
                                            uint16_t color) {
...
*/
inline void Adafruit_SPITFT_NG::writeFillRectPreclipped(int16_t x, int16_t y,
                                                     int16_t w, int16_t h,
                                                     uint16_t color) {
...
/*!
    ...
    @param  color  16-bit pixel color in '565' RGB format.
*/
void Adafruit_SPITFT_NG::drawPixel(int16_t x, int16_t y, uint16_t color) {
  // Clip first...
  if ((x >= 0) && (x < _width) && (y >= 0) && (y < _height)) {
...
/*!
    ...
    @note   This repeats the writeFillRect() function almost in its entirety,
            ...
            that much code.
*/
void Adafruit_SPITFT_NG::fillRect(int16_t x, int16_t y, int16_t w, int16_t h,
                               uint16_t color) {
  if (w && h) {   // Nonzero width and height?
    if (w < 0) {  // If negative width...
      x += w + 1; //   Move X to left edge
...
*/
void Adafruit_SPITFT_NG::drawFastHLine(int16_t x, int16_t y, int16_t w,
                                    uint16_t color) {
  if ((y >= 0) && (y < _height) && w) { // Y on screen, nonzero width
    if (w < 0) {                        // If negative width...
      x += w + 1;                       //   Move X to left edge
...
/*!
    ...
    @note   This repeats the writeFastVLine() function almost in its
            ...
            transaction isn't performed at all if the line is rejected.
*/
void Adafruit_SPITFT_NG::drawFastVLine(int16_t x, int16_t y, int16_t h,
                                    uint16_t color) {
  if ((x >= 0) && (x < _width) && h) { // X on screen, nonzero height
    if (h < 0) {                       // If negative height...
      y += h + 1;                      //   Move Y to top edge
...
/*!
    ...
    @param  color  16-bit pixel color in '565' RGB format.
*/
void Adafruit_SPITFT_NG::pushColor(uint16_t color) {
  startWrite();
  SPI_WRITE16(color);
  endWrite();
}
...
*/
void Adafruit_SPITFT_NG::drawRGBBitmap(int16_t x, int16_t y, uint16_t *pcolors,
                                    int16_t w, int16_t h) {
...
*/
void Adafruit_SPITFT_NG::invertDisplay(bool i) {
  startWrite();
...
*/
uint16_t Adafruit_SPITFT_NG::color565(uint8_t red, uint8_t green, uint8_t blue) {
  return ((red & 0xF8) << 8) | ((green & 0xFC) << 3) | (blue >> 3);
}

/*!
@brief   Adafruit_SPITFT_NG Send Command handles complete sending of commands and
data
@param   commandByte       The Command Byte
@param   dataBytes         A pointer to the Data bytes to send
@param   numDataBytes      The number of bytes we should send
*/
void Adafruit_SPITFT_NG::sendCommand(uint8_t commandByte, uint8_t *dataBytes,
...
/*!
 ...
 @param   numDataBytes      The number of bytes we should send
 */
void Adafruit_SPITFT_NG::sendCommand(uint8_t commandByte, const uint8_t *dataBytes,
                                  uint8_t numDataBytes) {
  SPI_BEGIN_TRANSACTION();
  if (_cs >= 0)
    SPI_CS_LOW();
...
/*!
 @brief  Adafruit_SPITFT_NG sendCommand16 handles complete sending of
 ...
 @param  numDataBytes  The number of bytes we should send
 */
void Adafruit_SPITFT_NG::sendCommand16(uint16_t commandWord,
 ...
 */
/**************************************************************************/
uint8_t Adafruit_SPITFT_NG::readcommand8(uint8_t commandByte, uint8_t index) {
  uint8_t result;
  startWrite();
  SPI_DC_LOW(); // Command mode
 ...
 */
uint16_t Adafruit_SPITFT_NG::readcommand16(uint16_t addr) {
...
*/
inline void Adafruit_SPITFT_NG::SPI_BEGIN_TRANSACTION(void) {
  if (connection == TFT_HARD_SPI) {
#if defined(SPI_HAS_TRANSACTION)
...
*/
inline void Adafruit_SPITFT_NG::SPI_END_TRANSACTION(void) {
#if defined(SPI_HAS_TRANSACTION)
  if (connection == TFT_HARD_SPI) {
...
*/
void Adafruit_SPITFT_NG::spiWrite(uint8_t b) {
  if (connection == TFT_HARD_SPI) {
#if defined(__AVR__)
    AVR_WRITESPI(b);
#elif defined(ESP8266) || defined(ESP32)
...
*/
void Adafruit_SPITFT_NG::writeCommand(uint8_t cmd) {
  SPI_DC_LOW();
  spiWrite(cmd);
  SPI_DC_HIGH();
}
...
*/
uint8_t Adafruit_SPITFT_NG::spiRead(void) {
  uint8_t b = 0;
  uint16_t w = 0;
  if (connection == TFT_HARD_SPI) {
...
*/
void Adafruit_SPITFT_NG::write16(uint16_t w) {
  if (connection == TFT_PARALLEL) {
#if defined(USE_FAST_PINIO)
    if (tft8.wide)
...
*/
void Adafruit_SPITFT_NG::writeCommand16(uint16_t cmd) {
  SPI_DC_LOW();
  write16(cmd);
  SPI_DC_HIGH();
}
...
/*!
    ...
    @return  Unsigned 16-bit value read (always zero if USE_FAST_PINIO is
             not supported by the MCU architecture).
*/
uint16_t Adafruit_SPITFT_NG::read16(void) {
  uint16_t w = 0;
  if (connection == TFT_PARALLEL) {
    if (tft8._rd >= 0) {
#if defined(USE_FAST_PINIO)
...
*/
inline void Adafruit_SPITFT_NG::SPI_MOSI_HIGH(void) {
#if defined(USE_FAST_PINIO)
#if defined(HAS_PORT_SET_CLR)
#if defined(KINETISK)
  *swspi.mosiPortSet = 1;
...
*/
inline void Adafruit_SPITFT_NG::SPI_MOSI_LOW(void) {
#if defined(USE_FAST_PINIO)
#if defined(HAS_PORT_SET_CLR)
#if defined(KINETISK)
  *swspi.mosiPortClr = 1;
...
*/
inline void Adafruit_SPITFT_NG::SPI_SCK_HIGH(void) {
#if defined(USE_FAST_PINIO)
#if defined(HAS_PORT_SET_CLR)
#if defined(KINETISK)
  *swspi.sckPortSet = 1;
...
*/
inline void Adafruit_SPITFT_NG::SPI_SCK_LOW(void) {
#if defined(USE_FAST_PINIO)
#if defined(HAS_PORT_SET_CLR)
#if defined(KINETISK)
  *swspi.sckPortClr = 1;
...
*/
inline bool Adafruit_SPITFT_NG::SPI_MISO_READ(void) {
#if defined(USE_FAST_PINIO)
#if defined(KINETISK)
...
/*!
    ...
    @param  w  16-bit value to write.
*/
void Adafruit_SPITFT_NG::SPI_WRITE16(uint16_t w) {
  if (connection == TFT_HARD_SPI) {
#if defined(__AVR__)
    AVR_WRITESPI(w >> 8);
    AVR_WRITESPI(w);
...
*/
void Adafruit_SPITFT_NG::SPI_WRITE32(uint32_t l) {
  if (connection == TFT_HARD_SPI) {
#if defined(__AVR__)
    AVR_WRITESPI(l >> 24);
    AVR_WRITESPI(l >> 16);
...
*/
inline void Adafruit_SPITFT_NG::TFT_WR_STROBE(void) {
#if defined(USE_FAST_PINIO)
#if defined(HAS_PORT_SET_CLR)
#if defined(KINETISK)
  *tft8.wrPortClr = 1;
...
/*!
    @brief  Set the RD line HIGH. Used for parallel-connected interfaces
            when reading data.
*/
inline void Adafruit_SPITFT_NG::TFT_RD_HIGH(void) {
#if defined(USE_FAST_PINIO)
#if defined(HAS_PORT_SET_CLR)
...
/*!
    @brief  Set the RD line LOW. Used for parallel-connected interfaces
            when reading data.
*/
inline void Adafruit_SPITFT_NG::TFT_RD_LOW(void) {
#if defined(USE_FAST_PINIO)
#if defined(HAS_PORT_SET_CLR)
