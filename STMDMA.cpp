...
STMDMA::STMDMA(Adafruit_ILI9341_NG *display, uint16_t bufferSize,
               SPI_TypeDef *spi, DMA_Stream_TypeDef *dma)
    : _display(display) {
  // Make sure our uint16_t state doesn't get overflowed
  if (bufferSize > 32767) bufferSize = 32767;
...
void STMDMA::fillRect(uint16_t x, uint16_t y, uint16_t w, uint16_t h,
  ...
  if (total < _bufferSize) {
    for (uint16_t i = 0; i < total; i++) _buffer[i] = color;
  } else {
    for (uint16_t i = 0; i < _bufferSize; i++) _buffer[i] = color;
  }
  while (blocks > 0) {
...
void STMDMA::beginWindow(uint16_t x, uint16_t y, uint16_t w, uint16_t h,
  ...
  for (uint16_t i = 0; i < _fl; i++) _buffer[i] = backgroundColor;
  _display->startWrite();
